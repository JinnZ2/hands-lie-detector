...
### The third gate — disposition

Two gates were named above. There is a third, and it closes the loop:

```
1  TRANSACTION GATE   no payment      → no record
2  AUTHORSHIP GATE    no seat         → no class
3  DISPOSITION GATE   no narration    → no text
```

A record externalizes only when it has a **consumer other than the person
holding it**. Someone who holds the record internally produces no text — not
from reluctance and not from oversight. The record was already complete where it
lived.

Generalized: **the corpus contains what people needed to transmit. It does not
contain what people knew.** Those two sets overlap only where knowledge had an
audience, and that overlap is not random.

Gate 3 is self-reinforcing in a specific way. People whose work *is* narrating
produce most of the corpus text about work — so "work," in the corpus, is
disproportionately described by people whose work is describing. The filter is
not on what people do. It is on **documentation propensity**, and propensity
